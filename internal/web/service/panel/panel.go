...
const (
	...
	defaultPanelReleaseTagURL = "https://api.github.com/repos/MHSanaei/3x-ui/releases/tags"
...
func (s *PanelService) startUpdate(useDev bool) error {
	...
	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		...
		if updateTag != "" {
			args = append(args, "--setenv", "XUI_UPDATE_TAG="+updateTag)
		}
...
func panelReleaseTagURL(tag string) string {
	base := strings.TrimRight(strings.TrimSpace(os.Getenv("XUI_RELEASE_TAG_API_URL")), "/")
	if base == "" {
		base = defaultPanelReleaseTagURL
	}
	return base + "/" + strings.TrimLeft(tag, "/")
}
...
func fetchPanelRelease(tag string) (*service.Release, error) {
	url := panelReleaseAPIURL()
	if tag != "" {
		url = panelReleaseTagURL(tag)
	}
	client := (&service.SettingService{}).NewProxiedHTTPClient(10 * time.Second)
	resp, err := panelHTTPGet(client, url)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
